...
- (void)layoutUserView:(int)gender {
    UIView *userView = [self userView];
    userView.tag = gender * 1000 + 1;                  //maleUser:1 femaleUser:1001
    
    if(gender == 0){
        userView.frame = CGRectMake((width - userView.frame.size.width) / 2,
                                   (height - userView.frame.size.height * 2) / 3 * 2 + userView.frame.size.height,
                                   userView.frame.size.width,
                                   userView.frame.size.height);
    } else {
        userView.frame = CGRectMake((width - userView.frame.size.width) / 2,
                                   (height - userView.frame.size.height * 2) / 3,
                                   userView.frame.size.width,
                                   userView.frame.size.height);
    }
